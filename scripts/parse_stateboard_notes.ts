...
const directoriesToProcess = [
  {
    inputDir: path.join(process.cwd(), 'stateboard notes'),
    outputDir: path.join(process.cwd(), 'public/notes/stateboard'),
    urlPrefix: '/notes/stateboard',
    boardCode: 'TNSB'
  }
];
              ...
              for (const topic of dbChapter.topics) {
                ...
                if (existingNote) {
                  await prisma.courseNote.update({
                    ...
                    data: { fileUrl, title: `Study Notes: ${topic.name}`, subjectTitle: file.replace('.md', '') }
                  });
                } else {
